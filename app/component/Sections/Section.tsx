...
import { ResizablePanel } from "@/components/ui/panelresize";
...
export default function Section({
  ...
  // -----------------------------------------
  function renderPanelContent(panel: PanelData) {
    const entry = panelRegistry[panel.panelProps.type];
    if (!entry) return <div>Unknown panel type</div>;

    const Component = entry.component;
    // Pass the full styling object to ensure all props are mapped correctly
    const mappedProps = entry.mapProps(panel.panelProps.content, {
      ...panel.styling,
      // Override container styles since ResizablePanel handles them
  ...
  // -----------------------------------------
  return (
    <div className="p-6 space-y-6 bg-gray-100 select-none" onClick={handleCanvasClick}>
      {/* Header */}
                ...
                {panelsInZone.map(panel => (
                  ...
                  >
                    <ResizablePanel
                      width={panel.w}
                      height={panel.h}
                      onResize={(size) => handlePanelResize(panel.i, size)}
                      isSelected={selectedPanelId === panel.i}
                      onSelect={() => handlePanelSelect(panel.i)}
                      minWidth={100}
                      minHeight={50}
                      className="rounded-lg overflow-hidden relative"
                      style={{
                        backgroundColor: panel.styling.backgroundColor,
                        opacity: panel.styling.opacity ?? 1,
                        borderRadius: `${panel.styling.borderRadius || 8}px`,
                    ...
                    >
                      ...
                      {/* Edit Button - Show on hover or when selected - Inside ResizablePanel */}
                      {(hoveredPanelId === panel.i || selectedPanelId === panel.i) && (
                        <button
                          onClick={(e) => handleEditClick(e, panel.i)}
                          className="!absolute !-top-0 !-right-0 !w-7 !h-7 !bg-blue-600 !hover:bg-blue-700 !rounded-full !shadow-lg !flex items-center !justify-center !transition-all !duration-200 z-[60] cursor-pointer"
